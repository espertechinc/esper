/*
 ***************************************************************************************
 *  Copyright (C) 2006 EsperTech, Inc. All rights reserved.                            *
 *  http://www.espertech.com/esper                                                     *
 *  http://www.espertech.com                                                           *
 *  ---------------------------------------------------------------------------------- *
 *  The software in this package is published under the terms of the GPL license       *
 *  a copy of which has been included with this distribution in the license.txt file.  *
 ***************************************************************************************
 */
package com.espertech.esper.regressionlib.support.bean;


import java.io.Serializable;

/**
 * Test event; only serializable because it *may* go over the wire  when running remote tests and serialization is just convenient. Serialization generally not used for HA and HA testing.
 */
public class SupportBeanKeywords implements Serializable {
    private static final long serialVersionUID = 7438931594766772054L;
    private int count = 1;
    private int escape = 1;
    private int every = 1;
    private int sum = 1;
    private int avg = 1;
    private int max = 1;
    private int min = 1;
    private int coalesce = 1;
    private int median = 1;
    private int stddev = 1;
    private int avedev = 1;
    private int events = 1;
    private int seconds = 1;
    private int minutes = 1;
    private int first = 1;
    private int last = 1;
    private int unidirectional = 1;
    private int pattern = 1;
    private int sql = 1;
    private int metadatasql = 1;
    private int prev = 1;
    private int prior = 1;
    private int weekday = 1;
    private int lastweekday = 1;
    private int cast = 1;
    private int snapshot = 1;
    private int variable = 1;
    private int window = 1;
    private int full = 1;
    private int inner = 1;
    private int outer = 1;
    private int left = 1;
    private int right = 1;
    private int join = 1;

    public int getCount() {
        return count;
    }

    public int getEscape() {
        return escape;
    }

    public int getEvery() {
        return every;
    }

    public int getSum() {
        return sum;
    }

    public int getAvg() {
        return avg;
    }

    public int getMax() {
        return max;
    }

    public int getMin() {
        return min;
    }

    public int getCoalesce() {
        return coalesce;
    }

    public int getMedian() {
        return median;
    }

    public int getStddev() {
        return stddev;
    }

    public int getAvedev() {
        return avedev;
    }

    public int getEvents() {
        return events;
    }

    public int getSeconds() {
        return seconds;
    }

    public int getMinutes() {
        return minutes;
    }

    public int getFirst() {
        return first;
    }

    public int getLast() {
        return last;
    }

    public int getUnidirectional() {
        return unidirectional;
    }

    public int getPattern() {
        return pattern;
    }

    public int getSql() {
        return sql;
    }

    public int getMetadatasql() {
        return metadatasql;
    }

    public int getPrev() {
        return prev;
    }

    public int getPrior() {
        return prior;
    }

    public int getWeekday() {
        return weekday;
    }

    public int getLastweekday() {
        return lastweekday;
    }

    public int getCast() {
        return cast;
    }

    public int getSnapshot() {
        return snapshot;
    }

    public int getVariable() {
        return variable;
    }

    public int getWindow() {
        return window;
    }

    public int getFull() {
        return full;
    }

    public int getInner() {
        return inner;
    }

    public int getOuter() {
        return outer;
    }

    public int getLeft() {
        return left;
    }

    public int getRight() {
        return right;
    }

    public int getJoin() {
        return join;
    }
}
