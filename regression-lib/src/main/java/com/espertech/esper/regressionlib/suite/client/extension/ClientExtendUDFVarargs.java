...
public class ClientExtendUDFVarargs {
    ...
    private static class ClientExtendUDFVarargsCollOfEvent implements RegressionExecution {
        public void run(RegressionEnvironment env) {
            String epl = "@name('s0') select " + ClientExtendUDFVarargs.class.getName() + ".mySupportVarArgsMethod(1, " +
    ...
    private static class ClientExtendUDFVarargsCombinations implements RegressionExecution {
        public void run(RegressionEnvironment env) {
    ...
    private static void runVarargAssertion(RegressionEnvironment env, AtomicInteger milestone, UniformPair<String>... pairs) {
    ...
    private static void tryAssertionArraysAsList(RegressionEnvironment env, AtomicInteger milestone) {
    ...
    private static void assertEqualsColl(EventBean event, String property, String... values) {
        Collection data = (Collection) event.get(property);
        EPAssertionUtil.assertEqualsExactOrder(values, data.toArray());
    }

    public static Object[] mySupportVarArgsMethod(Object... varargs) {
        return varargs;
    }
}
