...
public class EvalEveryDistinctStateExpireKeyNode extends EvalStateNode implements Evaluator {
    ...
    public void removeMatch(Set<EventBean> matchEvent) {
        if (PatternConsumptionUtil.containsEvent(matchEvent, beginState)) {
            quit();
            AgentInstanceContext agentInstanceContext = everyNode.getContext().getAgentInstanceContext();
            agentInstanceContext.getAuditProvider().patternFalse(everyNode.getFactoryNode(), this, agentInstanceContext);
    ...
    public final void start(MatchedEventMap beginState) {
        AgentInstanceContext agentInstanceContext = everyNode.getContext().getAgentInstanceContext();
        agentInstanceContext.getInstrumentationProvider().qPatternEveryDistinctStart(everyNode.factoryNode, beginState);
        agentInstanceContext.getAuditProvider().patternInstance(true, everyNode.factoryNode, agentInstanceContext);

        this.beginState = beginState.shallowCopy();
        EvalStateNode childState = everyNode.getChildNode().newState(this);
    ...
    public final void evaluateFalse(EvalStateNode fromNode, boolean restartable) {
        AgentInstanceContext agentInstanceContext = everyNode.getContext().getAgentInstanceContext();
        agentInstanceContext.getInstrumentationProvider().qPatternEveryDistinctEvalFalse(everyNode.factoryNode);
        ...
        EvalEveryStateSpawnEvaluator spawnEvaluator = new EvalEveryStateSpawnEvaluator(everyNode.getContext().getStatementName());
        EvalStateNode spawned = everyNode.getChildNode().newState(spawnEvaluator);
    ...
    public final void evaluateTrue(MatchedEventMap matchEvent, EvalStateNode fromNode, boolean isQuitted, EventBean optionalTriggeringEvent) {
        AgentInstanceContext agentInstanceContext = everyNode.getContext().getAgentInstanceContext();
        agentInstanceContext.getInstrumentationProvider().qPatternEveryDistinctEvaluateTrue(everyNode.factoryNode, matchEvent);

        // determine if this evaluation has been seen before from the same node
        Object matchEventKey = PatternExpressionUtil.getKeys(matchEvent, everyNode.getFactoryNode().getConvertor(), everyNode.getFactoryNode().getDistinctExpression(), everyNode.getContext().getAgentInstanceContext());
        ...
        if (keysFromNode != null) {
            ...
            long currentTime = everyNode.getContext().getAgentInstanceContext().getTimeProvider().getTime();
            for (; it.hasNext(); ) {
        ...
        if (!haveSeenThis) {
            agentInstanceContext.getAuditProvider().patternTrue(everyNode.getFactoryNode(), this, matchEvent, false, agentInstanceContext);
    ...
    public final void quit() {
        AgentInstanceContext agentInstanceContext = everyNode.getContext().getAgentInstanceContext();
        agentInstanceContext.getInstrumentationProvider().qPatternEveryDistinctQuit(everyNode.factoryNode);
        agentInstanceContext.getAuditProvider().patternInstance(false, everyNode.factoryNode, agentInstanceContext);
