...
public abstract class ExprDotForgeTwoLambda extends ExprDotForgeEnumMethodBase {
    ...
    protected abstract EPType returnType(EventType inputEventType, Class collectionComponentType);
    ...
    public EnumForgeDescFactory getForgeFactory(DotMethodFP footprint, List<ExprNode> parameters, EnumMethodEnum enumMethod, String enumMethodUsedName, EventType inputEventType, Class collectionComponentType, ExprValidationContext validationContext)
        ...
        if (inputEventType != null) {
            ...
            fieldsFirst.put(lambdaFirst.getGoesToNames().get(1), Integer.class);
            fieldsSecond.put(lambdaSecond.getGoesToNames().get(1), Integer.class);
            if (numParameters > 2) {
                fieldsFirst.put(lambdaFirst.getGoesToNames().get(2), Integer.class);
                fieldsSecond.put(lambdaSecond.getGoesToNames().get(2), Integer.class);
            }
            ...
            return new TwoLambdaThreeFormEventPlusFactory(inputEventType, streamNameFirst, streamNameSecond, typeFirst, typeSecond, lambdaFirst.getGoesToNames().size(), returnType, twoParamEventPlus());
        }
        ...
        ObjectArrayEventType typeFirst = ExprDotNodeUtility.makeTransientOAType(enumMethodUsedName, fieldsFirst, validationContext.getStatementRawInfo(), validationContext.getStatementCompileTimeService());
        ObjectArrayEventType typeSecond = ExprDotNodeUtility.makeTransientOAType(enumMethodUsedName, fieldsSecond, validationContext.getStatementRawInfo(), validationContext.getStatementCompileTimeService());
        return new TwoLambdaThreeFormScalarFactory(typeFirst, typeSecond, lambdaFirst.getGoesToNames().size(), returnType, twoParamScalar());
    }
}
