...
public class HistoricalIndexLookupStrategyInKeywordSingle implements HistoricalIndexLookupStrategy {
    public final static EPTypeClass EPTYPE = new EPTypeClass(HistoricalIndexLookupStrategyInKeywordSingle.class);
    ...
    public Iterator<EventBean> lookup(EventBean lookupEvent, EventTable[] index, ExprEvaluatorContext context) {
        if (index[0] instanceof PropertyHashedEventTable) {
            ...
            Set<EventBean> result = InKeywordTableLookupUtil.singleIndexLookup(evaluators, eventsPerStream, context, idx);
            if (result == null) {
                return null;
            }
