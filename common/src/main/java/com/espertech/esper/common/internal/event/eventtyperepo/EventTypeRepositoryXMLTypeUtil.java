...
public class EventTypeRepositoryXMLTypeUtil {
    public static void buildXMLTypes(EventTypeRepositoryImpl repo, Map<String, ConfigurationCommonEventTypeXMLDOM> eventTypesXMLDOM, BeanEventTypeFactory beanEventTypeFactory, XMLFragmentEventTypeFactory xmlFragmentEventTypeFactory, ClasspathImportService classpathImportService, EventTypeXMLXSDHandler xmlxsdHandler) {
        ...
        for (Map.Entry<String, ConfigurationCommonEventTypeXMLDOM> entry : eventTypesXMLDOM.entrySet()) {
            ...
            if ((entry.getValue().getSchemaResource() != null) || (entry.getValue().getSchemaText() != null)) {
                try {
                    schemaModel = xmlxsdHandler.loadAndMap(entry.getValue().getSchemaResource(), entry.getValue().getSchemaText(), classpathImportService);
                } catch (Exception ex) {
            ...
            try {
                addXMLDOMType(repo, entry.getKey(), entry.getValue(), schemaModel, beanEventTypeFactory, xmlFragmentEventTypeFactory, xmlxsdHandler);
            } catch (Throwable ex) {
                throw new ConfigurationException(ex.getMessage(), ex);
            }
        }
    ...
    private static void addXMLDOMType(EventTypeRepositoryImpl repo, String eventTypeName, ConfigurationCommonEventTypeXMLDOM detail, SchemaModel schemaModel, BeanEventTypeFactory beanEventTypeFactory, XMLFragmentEventTypeFactory xmlFragmentEventTypeFactory, EventTypeXMLXSDHandler xmlxsdHandler) {
        ...
        EventType type = beanEventTypeFactory.getEventTypeFactory().createXMLType(metadata, detail, schemaModel, null, metadata.getName(), beanEventTypeFactory, xmlFragmentEventTypeFactory, repo, xmlxsdHandler);
        repo.addType(type);
