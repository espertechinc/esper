...
public class StmtClassForgableMultiKey implements StmtClassForgable {
    ...
    public StmtClassForgableMultiKey(String className, CodegenPackageScope packageScope, Class[] types, boolean lenientEquals) {
    ...
    public CodegenClass forge(boolean includeDebugSymbols) {
        ...
        CodegenCtor ctor = new CodegenCtor(StmtClassForgableMultiKey.class, includeDebugSymbols, params);

        CodegenClassMethods methods = new CodegenClassMethods();
        CodegenClassScope classScope = new CodegenClassScope(includeDebugSymbols, packageScope, className);

        CodegenMethod hashMethod = CodegenMethod.makeParentNode(int.class, StmtClassForgableMultiKey.class, CodegenSymbolProviderEmpty.INSTANCE, classScope);
        makeHashMethod(types.length, hashMethod);
        CodegenStackGenerator.recursiveBuildStack(hashMethod, "hashCode", methods);

        CodegenMethod equalsMethod = CodegenMethod.makeParentNode(boolean.class, StmtClassForgableMultiKey.class, CodegenSymbolProviderEmpty.INSTANCE, classScope).addParam(Object.class, "o");
        makeEqualsMethod(types.length, equalsMethod);
        CodegenStackGenerator.recursiveBuildStack(equalsMethod, "equals", methods);

        CodegenMethod getNumKeysMethod = CodegenMethod.makeParentNode(int.class, StmtClassForgableMultiKey.class, CodegenSymbolProviderEmpty.INSTANCE, classScope);
        ...
        CodegenMethod getKeyMethod = CodegenMethod.makeParentNode(Object.class, StmtClassForgableMultiKey.class, CodegenSymbolProviderEmpty.INSTANCE, classScope).addParam(int.class, "num");
        makeGetKeyMethod(types.length, getKeyMethod);
        CodegenStackGenerator.recursiveBuildStack(getKeyMethod, "getKey", methods);

        CodegenMethod toStringMethod = CodegenMethod.makeParentNode(String.class, StmtClassForgableMultiKey.class, CodegenSymbolProviderEmpty.INSTANCE, classScope);
        makeToStringMethod(toStringMethod);
        CodegenStackGenerator.recursiveBuildStack(toStringMethod, "toString", methods);

        return new CodegenClass(CodegenClassType.KEYPROVISIONING, MultiKeyGenerated.class, className, classScope, Collections.emptyList(), ctor, methods, Collections.emptyList());
    }
    ...
    private void makeEqualsMethod(int length, CodegenMethod equalsMethod) {
        ...
        */
        ...
        equalsMethod.getBlock().ifCondition(not(instanceOf(ref("o"), MultiKeyGenerated.class))).blockReturn(constant(false))
            .declareVar(MultiKeyGenerated.class, "k", cast(MultiKeyGenerated.class, ref("o")));
    ...
    private void makeToStringMethod(CodegenMethod toStringMethod) {
        toStringMethod.getBlock()
            ...
            .exprDotMethod(ref("b"), "append", constant(MultiKeyGenerated.class.getSimpleName() + "["));
        for (int i = 0; i < types.length; i++) {
            if (i > 0) {
