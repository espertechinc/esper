...
public class EvalAndStateNode extends EvalStateNode implements Evaluator {
    ...
    public void removeMatch(Set<EventBean> matchEvent) {
        ...
        if (quit) {
            quit();
            AgentInstanceContext agentInstanceContext = evalAndNode.getContext().getAgentInstanceContext();
            agentInstanceContext.getAuditProvider().patternFalse(evalAndNode.getFactoryNode(), this, agentInstanceContext);
    ...
    public final void start(MatchedEventMap beginState) {
        AgentInstanceContext agentInstanceContext = evalAndNode.getContext().getAgentInstanceContext();
        agentInstanceContext.getInstrumentationProvider().qPatternAndStart(evalAndNode.factoryNode, beginState);
        agentInstanceContext.getAuditProvider().patternInstance(true, evalAndNode.factoryNode, agentInstanceContext);
    ...
    public final void evaluateTrue(MatchedEventMap matchEvent, EvalStateNode fromNode, boolean isQuitted, EventBean optionalTriggeringEvent) {
        AgentInstanceContext agentInstanceContext = evalAndNode.getContext().getAgentInstanceContext();
        agentInstanceContext.getInstrumentationProvider().qPatternAndEvaluateTrue(evalAndNode.factoryNode, matchEvent);
        ...
        if (quitted) {
            agentInstanceContext.getAuditProvider().patternInstance(false, evalAndNode.factoryNode, agentInstanceContext);
            quitInternal();
        }

        // Send results to parent
        for (MatchedEventMap theEvent : result) {
            agentInstanceContext.getAuditProvider().patternTrue(evalAndNode.getFactoryNode(), this, theEvent, quitted, agentInstanceContext);
    ...
    public final void evaluateFalse(EvalStateNode fromNode, boolean restartable) {
        AgentInstanceContext agentInstanceContext = evalAndNode.getContext().getAgentInstanceContext();
        agentInstanceContext.getInstrumentationProvider().qPatternAndEvaluateFalse(evalAndNode.factoryNode);
    ...
    public final void quit() {
        ...
        AgentInstanceContext agentInstanceContext = evalAndNode.getContext().getAgentInstanceContext();
        agentInstanceContext.getInstrumentationProvider().qPatternAndQuit(evalAndNode.factoryNode);
        agentInstanceContext.getAuditProvider().patternInstance(false, evalAndNode.factoryNode, agentInstanceContext);
        quitInternal();
