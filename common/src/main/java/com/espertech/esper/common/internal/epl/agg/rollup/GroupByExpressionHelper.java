    ...
    public static GroupByClauseExpressions getGroupByRollupExpressions(List<GroupByClauseElement> groupByElements,
        ...
        for (int i = 0; i < numberOfLevels; i++) {
            ...
            for (int j = 0; j < expressions.size(); j++) {
                ...
                selects[i][j] = copyVisitExpression(compiled.getSelectExpression(), expressionCopier);
            }
        }
        ...
        if (orderByList != null && orderByList.size() > 0) {
            ...
            for (int i = 0; i < numberOfLevels; i++) {
                ...
                for (int j = 0; j < orderByList.size(); j++) {
                    ...
                    optOrderByCopy[i][j] = copyVisitExpression(element.getExprNode(), expressionCopier);
                }
            }
        }
