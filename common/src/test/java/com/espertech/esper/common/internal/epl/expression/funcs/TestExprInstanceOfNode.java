...
public class TestExprInstanceOfNode extends TestCase {
    ...
    public void testGetType() throws Exception {
        for (int i = 0; i < instanceofNodes.length; i++) {
            ...
            assertEquals(EPTypePremade.BOOLEANBOXED.getEPType(), instanceofNodes[i].getForge().getEvaluationType());
        }
    }
    ...
    public void testToExpressionString() throws Exception {
        assertEquals("instanceof(\"\"," + SupportBean.class.getName() + ",int,string)", ExprNodeUtilityPrint.toExpressionStringMinPrecedenceSafe(instanceofNodes[1]));
    }
}
