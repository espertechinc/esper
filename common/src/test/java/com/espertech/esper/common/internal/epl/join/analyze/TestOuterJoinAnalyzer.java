...
public class TestOuterJoinAnalyzer extends TestCase {
    public void testAnalyze() throws Exception {
        ...
        QueryGraphForge graph = new QueryGraphForge(3, null, false);
        OuterJoinAnalyzer.analyze(descList, graph);
        assertEquals(3, graph.getNumStreams());

        assertTrue(graph.isNavigableAtAll(0, 1));
        assertEquals(1, SupportQueryGraphTestUtil.getStrictKeyProperties(graph, 0, 1).length);
        assertEquals("intPrimitive", SupportQueryGraphTestUtil.getStrictKeyProperties(graph, 0, 1)[0]);
        assertEquals(1, SupportQueryGraphTestUtil.getStrictKeyProperties(graph, 1, 0).length);
        assertEquals("intBoxed", SupportQueryGraphTestUtil.getStrictKeyProperties(graph, 1, 0)[0]);

        assertTrue(graph.isNavigableAtAll(1, 2));
        assertEquals("theString", SupportQueryGraphTestUtil.getStrictKeyProperties(graph, 1, 2)[0]);
        assertEquals("simpleProperty", SupportQueryGraphTestUtil.getStrictKeyProperties(graph, 2, 1)[0]);
    }
}
