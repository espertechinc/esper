...
import static com.espertech.esperio.kafka.SupportCompileUtil.undeployAll;
...
public class TestKafkaOutput extends TestCase {
    ...
    public void testOutput() {
        ...
        adapterProps.put(ProducerConfig.CLIENT_ID_CONFIG, TestKafkaOutput.class.getName() + "-" + UuidGenerator.generate());
        ...
        EPRuntime runtime = SupportConstants.getEngineWKafkaOutput(this.getClass().getSimpleName(), adapterProps);

        // create statement
        compileDeploy(runtime, "@name('first') @KafkaOutputDefault select * from SupportBean");
        ...
        sendAndAwait(runtime, consumer, "E1");
        sendAndAwait(runtime, consumer, "E2");

        // destroy and check nothing received
        int numMessagesAfter = countMessages(topicPartitions, consumer);
        undeployAll(runtime);

        runtime.getEventService().sendEventBean(new SupportBean("XXX", -1), "SupportBean");
        assertEquals(2, numMessagesAfter - numMessages);

        // create second statement
        compileDeploy(runtime, "@name('second') @KafkaOutputDefault select stringProp from SupportBean");

        // send-and-await another event
        sendAndAwait(runtime, consumer, "E3");
    ...
    private void sendAndAwait(EPRuntime runtime, KafkaConsumer<String, String> consumer, String eventId) {
        // send event
        String uniqueId = eventId + "___" + UUID.randomUUID().toString();
        runtime.getEventService().sendEventBean(new SupportBean(uniqueId, 10), "SupportBean");
