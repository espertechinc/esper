...
import com.espertech.esper.example.servershell.jmx.EPRuntimeJMX;
...
public class ServerShellMain {
    ...
    public ServerShellMain() throws Exception {
        ...
        // Initialize runtime
        log.info("Getting Esper runtime instance");
        Configuration configuration = new Configuration();
        configuration.getCommon().addEventType("SampleEvent", SampleEvent.class.getName());
        EPRuntime runtime = EPRuntimeProvider.getDefaultRuntime(configuration);

        // Initialize statement
        log.info("Creating sample statement");
        SampleStatement.createStatement(runtime);

        // Register MBean
        log.info("Registering MBean");
        ObjectName name = new ObjectName(ServerShellConstants.MGMT_MBEAN_NAME);
        EPRuntimeJMX mbean = new EPRuntimeJMX(runtime);
        ...
        for (int i = 0; i < numListeners; i++) {
            listeners[i] = new SampleJMSMessageListener(runtime.getEventService());
