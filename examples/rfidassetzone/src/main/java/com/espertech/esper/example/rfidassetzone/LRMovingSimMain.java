...
public class LRMovingSimMain implements Runnable {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length < 3) {
            ...
            System.out.println("  number of threads: the number of threads sending events into the runtime(e.g. 4)");
    ...
    public LRMovingSimMain(int numberOfThreads, int numberOfAssetGroups, int numberOfSeconds, boolean isAssert, String runtimeURI, boolean continuousSimulation) {
        ...
        config.getCommon().addEventType(LocationReport.class);
        config.getCommon().addEventType(AssetGroupingEvent.class);

        runtime = EPRuntimeProvider.getRuntime(runtimeURI, config);
        runtime.initialize();
    }
    ...
    private void tryPerf(int numSeconds, int numAssetGroups, int numThreads, int ratioZoneMove, int ratioZoneSplit) {
        ...
        String epl =
            "// Each asset group initialized from an AssetGroupingEvent\n" +
                ...
                "from LocationReport(assetId in (context.grp.assetIdsGrouped))#unique(assetId) \n" +
                "group by zone;\n" +
                "\n" +
                "// Alerts when counts indicate that assets stay separated for a given group of assets\n" +
                "@name('out') context AssetGroupContext select * from pattern [every a=AssetCounts(groupId=context.grp.groupId and cnt in [1:2]) ->" +
                "  (timer:interval(10 sec) and not AssetCounts(groupId=context.grp.groupId and cnt in (0, 3)))]";
        ...
        try {
            ...
            throw new RuntimeException("Failed to compile: " + ex.getMessage(), ex);
        }

        // deploy
        try {
            runtime.getDeploymentService().deploy(compiled, new DeploymentOptions().setDeploymentId("assetmonitor"));
        } catch (EPDeployException ex) {
            throw new RuntimeException("Failed to deploy: " + ex.getMessage(), ex);
        }

        // attach listener
        EPStatement output = runtime.getDeploymentService().getStatement("assetmonitor", "out");
        AssetZoneSplitListener listener = new AssetZoneSplitListener();
        output.addListener(new AssetZoneSplitListener());

        // allocate groups
        for (int i = 0; i < numAssetGroups; i++) {
            AssetGroupingEvent groupingEvent = new AssetGroupingEvent(i, assetIds[i]);
            runtime.getEventService().sendEventBean(groupingEvent, "AssetGroupingEvent");
        }
        ...
        if (continuousSimulation) {
            while (true) {
                AssetEventGenCallable callable = new AssetEventGenCallable(runtime, assetIds, zoneIds, assetGroupsForThread[0], ratioZoneMove, ratioZoneSplit);
                try {
                    callable.call();
                } catch (Exception ex) {
        ...
        for (int i = 0; i < numThreads; i++) {
            callables[i] = new AssetEventGenCallable(runtime, assetIds, zoneIds, assetGroupsForThread[i], ratioZoneMove, ratioZoneSplit);
        ...
        for (Integer groupId : splitGroups) {
            if (!listener.getCallbacks().contains(groupId)) {
