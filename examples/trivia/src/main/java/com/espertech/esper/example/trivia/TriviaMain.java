...
public class TriviaMain {
    ...
    public void run() {
        ...
        for (int i = 0; i < rounds.size(); i++) {
            ...
            // set time
            setTime(runtime, round.getTime());

            // send question event
            runtime.getEventService().sendEventMap(round.getQuestion(), "TriviaQuestion");

            // send player activity
            for (PlayerAction action : round.getPlayerActions()) {
                runtime.getEventService().sendEventMap(action.getData(), action.getEventName());
            }

            // trigger score update
            runtime.getEventService().sendEventMap(EventFactory.makeUpdateScore(round.getQuestionId()), "UpdateScore");

            printScore(round.getQuestionId(), TriviaHelper.getScores(runtime));
        }
    ...
    private static void setTime(EPRuntime runtime, String time) {
