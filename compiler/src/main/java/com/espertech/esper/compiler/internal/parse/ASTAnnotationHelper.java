...
public class ASTAnnotationHelper {
    ...
    private static Object walkClassIdent(EsperEPL2GrammarParser.ClassIdentifierContext ctx, ClasspathImportServiceCompileTime classpathImportService) {
        ...
        try {
            enumValueAndField = ClasspathImportCompileTimeUtil.resolveIdentAsEnumConst(enumValueText, classpathImportService, ClasspathExtensionClassEmpty.INSTANCE, true);
        ...
        if (enumValueText.endsWith(".class") && enumValueText.length() > 6) {
            try {
                ...
                enumValue = classpathImportService.resolveClass(name, true, ClasspathExtensionClassEmpty.INSTANCE);
