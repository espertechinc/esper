...
public class CompilerServicesImpl implements CompilerServices {
    ...
    public Class compileStandInClass(CodegenClassType classType, String classNameSimple, ModuleCompileTimeServices services) {
        ...
        CompilerAbstractionCompilationContext context = new CompilerAbstractionCompilationContext(services, Collections.emptyList());
        CompilerAbstractionClassCollection state = services.getCompilerAbstraction().newClassCollection();
        services.getCompilerAbstraction().compileClasses(Collections.singletonList(clazz), context, state);

        ByteArrayProvidingClassLoader cl = new ByteArrayProvidingClassLoader(state.getClasses(), services.getParentClassLoader());
