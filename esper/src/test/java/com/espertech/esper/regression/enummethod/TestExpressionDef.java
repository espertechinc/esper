...
public class TestExpressionDef extends TestCase {
    ...
    public void testNestedExpressionMultiSubquery() {
        ...
        epService.getEPAdministrator().createEPL("create expression F1 { (select intPrimitive from SupportBean.std:lastevent())}");
        epService.getEPAdministrator().createEPL("create expression F2 { param => (select a.intPrimitive from SupportBean.std:unique(theString) as a where a.theString = param.theString) }");
    ...
    public void testSubqueryCross() {
        ...
        String eplAlias = "expression subq alias for { (select theString from SupportBean.win:keepall() where theString = one.id and intPrimitive = two.p10) }" +
                "select subq as val1 " +
                "from SupportBean_ST0.std:lastevent() as one, SupportBean_ST1.std:lastevent() as two";
    ...
    public void testSubqueryCorrelated() {
        ...
        String eplAlias = "expression subqOne alias for {(select id from SupportBean_ST0.win:keepall() where p00 = t.intPrimitive)} " +
                "select theString as val0, subqOne(t) as val1 from SupportBean as t";
    ...
    public void testSubqueryUncorrelated() {
        String eplDeclare = "expression subqOne {(select id from SupportBean_ST0.std:lastevent())} " +
                "select theString as val0, subqOne() as val1 from SupportBean as t";
        runAssertionSubqueryUncorrelated(eplDeclare);

        String eplAlias = "expression subqOne alias for {(select id from SupportBean_ST0.std:lastevent())} " +
                "select theString as val0, subqOne as val1 from SupportBean as t";
