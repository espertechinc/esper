...
import com.espertech.esper.regressionlib.suite.expr.clazz.ExprClassDisable;
import com.espertech.esper.regressionrun.runner.RegressionRunner;
import com.espertech.esper.regressionrun.runner.RegressionSession;
import junit.framework.TestCase;

public class TestSuiteExprClazzWConfig extends TestCase {
    public void testEPLScriptExpressionDisable() {
        RegressionSession session = RegressionRunner.session();
        session.getConfiguration().getCommon().addEventType(SupportBean.class);
        session.getConfiguration().getCompiler().getByteCode().setAllowInlinedClass(false);
